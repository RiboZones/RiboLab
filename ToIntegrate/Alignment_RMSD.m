...
KeepAln3A=~ismember(cellstr(RV_Lab_Struct3.RiboLabCads_rRNA(1).Alignment(Species_Ind1).Sequence')','-');
KeepAln3B=~ismember(cellstr(RV_Lab_Struct3.RiboLabCads_rRNA(1).Alignment(Species_Ind2).Sequence')','-');
...
for iii=1:length(Aln_Struct(1).Sequence)
    ...
    if Aln_Struct(2).Sequence(iii) ~= '-'
        jj=jj+1;
        AlnMap{2,iii}=RV_Lab_Struct2.RiboLabMap.ItemNames{jj};
    end
...
RL1=RV_Lab_Struct1.RiboLabMap.ItemNames(KeepAln1 & ~ RemoveAln3_A);
RL2=RV_Lab_Struct2.RiboLabMap.ItemNames(KeepAln2 & ~ RemoveAln3_B);
% RemoveResi=RV_Lab_Struct1.RiboLabMap.ItemNames(KeepAln1 & RemoveAln3_A);
RL3=RV_Lab_Struct3.RiboLabMap.ItemNames(KeepAln3);
...
RMSD_Full1(ismember(RV_Lab_Struct1.RiboLabMap.ItemNames,RL1(all_xtals)))=RMSD;
RMSD_Full2(ismember(RV_Lab_Struct2.RiboLabMap.ItemNames,RL2(all_xtals)))=RMSD;
