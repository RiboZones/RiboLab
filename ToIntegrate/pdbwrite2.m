function pdbwrite(filename, pdbstruct)
...
%   coordinate information is store in the Model field of PDBSTRUCT. In the
...
%   $Revision: 1.1.6.10 $  $Date: 2009/01/30 14:40:32 $
...
% Validate input data
if nargin < 2
    error('Bioinfo:pdbwrite:NotEnoughInputs',...
        '%s requires at least two input arguments.',mfilename);
end

% Check filename is string.
if ~ischar(filename) || isempty(filename),
    error('Bioinfo:pdbwrite:InvalidFileName',...
        'The first input argument FILENAME must be a non-empty string.');
end

% Check pdbstruct is a structure.
if ~isstruct(pdbstruct)
    error('Bioinfo:pdbwrite:InvalidInputStructure',...
        'The second input argument must be a MATLAB structure.')
end
...
if ~all(m)
    ...
    if numel(missingFields)== numel(MandatoryFields)
        warning ('Bioinfo:pdbwrite:MissingAllMandatoryFields',...
                'All the mandatory fields are missing from the input PDB structure.'); 
    else
        ...
        warning('Bioinfo:pdbwrite:MissingMandatoryField',...
            'The following mandatory field(s) are missing from the input PDB structure: %s.', mf);
...
for i=1:no_of_chains
    ...
    if(no_of_lines)
        ...
        y(:,1:71)= [reshape(sprintf('SEQRES  %3d ',1:no_of_lines),12,no_of_lines)' ...
            repmat(sprintf('%c %4d  ',...
            m_seqres(i).ChainID,...
    ...
    if(r)
        ...
        x = [sprintf('SEQRES  %3d %c %4d  ',no_of_lines+1,m_seqres(i).ChainID,...
            m_seqres(i).NumOfResidues) full_seq(len_of_chain-r+1:len_of_chain)];
        z(lc:lc+no_of_lines,:)=[y;[x blanks(50-r+10)]];
        lc = lc + no_of_lines + 1;
        y=[];
    end
end
...
if(noOfLines > 1)
    z(2:end,8:10) = reshape(sprintf('%3d',2:noOfLines),3,noOfLines-1)';
