function [ RV_Lab_Structs, H_CoVar, F_CoVar,Dyads ] = PaseEntropyStruct( RV_Lab_Structs, varargin )
...
for jjj=1:length(RV_Lab_Structs)
    ...
    [H_CoVar, F_CoVar,MostFreqBasePair,Dyads{jjj}]=PASE(Alignment,RV_Lab_Structs.ResidueList,...
        RV_Lab_Structs(jjj).FR3D_Interaction_Tables,BP_Types,varargin{:});
    ...
    if MergeSingles
        ...
        Hadj=zeros(1,lenSeq);
        if SimpleMerge
            Hadj(H_CoVar < 0) = H(H_CoVar < 0);
            Hadj(H_CoVar > 0) = ScaleFactor * H_CoVar(H_CoVar > 0);
        else
            Hadj=H;
            Hadj(H_CoVar >= 0) = min([H(H_CoVar >= 0); ScaleFactor * H_CoVar(H_CoVar >= 0)]);
        end
    else
        Hadj=H_CoVar;
    end
    ...
    RV_Lab_Structs(jjj).CoVarEntropy_Table=[RV_Lab_Structs(jjj).ResidueList(1:length(Hadj)),...
        num2cell(Hadj'),MostFreqBasePair'];
