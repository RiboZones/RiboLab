...
%   $Revision: 1.11.4.24 $  $Date: 2010/06/03 15:56:40 $

% Validate input data
if nargin < 1
    error('Bioinfo:pdbread:NotEnoughInputs',...
        '%s requires at least one input arguments.',mfilename);
end

% Check pdbfile is a string.
if ~ischar(pdbfile)
    error('Bioinfo:pdbread:InvalidInput',...
...
elseif (strfind(pdbfile, '://'))
    if (~usejava('jvm'))
        error('Bioinfo:pdbread:NoJava', 'Reading from a URL requires Java.')
    end
    % Must be a URL
    ...
    error('Bioinfo:pdbread:BadPDBFile',...
...
if  nargin > 1
    if rem(nargin,2) ~= 1
        error('Bioinfo:pdbread:IncorrectNumberOfArguments',...
    ...
    for j=1:2:nargin-1
        ...
        if isempty(k)
            error('Bioinfo:pdbread:UnknownParameterName',...
                'Unknown parameter name: %s.',pname);
        elseif length(k)>1
            error('Bioinfo:pdbread:AmbiguousParameterName',...
            ...
            switch(k)
                case 1  % model number
                    ...
                    if ~isnumeric(modelNum) || numel(modelNum)> 1 || isempty(modelNum) || modelNum < 0
                        error('Bioinfo:pdbread:BadModelNumber',...
                            'ModelNumber must be a positive numeric scalar.')
                    end
            end
        end
    end
...
if fid == -1,
    if ~useTempFile
        error('Bioinfo:pdbread:CouldNotOpenFile',...
            'Could not open file %s.', filename);
    else
        error('Bioinfo:pdbread:CouldNotOpenTempFile',...
            'Could not open temporary file %s.',filename);
    end
end
        ...
        case 'ATOM'
            ...
            NumOfAtom = NumOfAtom+1;
                ...
                struct('AtomSerNo',{str2int(tline(7:11))},...
                       ...
                       'occupancy',{str2float(tline(55:60))},...
                       'tempFactor',{str2float(tline(61:66))},...
                       'segID',{tline(73:76)},...
                       'element',{strtrim(tline(77:78))},...
                       'charge',{tline(79:80)},...
                       'AtomNameStruct',struct('wholeAtomName',{strtrim(tline(13:16))},...
                                               'chemSymbol',{strtrim(tline(77:78))},...
