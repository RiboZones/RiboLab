...
%   $Revision: 1.1.8.20 $  $Date: 2010/12/22 16:17:06 $
    ...
    for j=1:2:nargin-2
        ...
        elseif length(k)>1
            ...
            switch(k)
                ...
                case 7 %format
                    if ismember(pval,{'text','xml'})
                        format=['&FORMAT_TYPE=',pval];
                    end
            end
        end
    end
            ...
            if strcmpi(fileformat,'html')
                try
                    ...
                    if strcmpi(theException.identifier,'MATLAB:urlwrite:InvalidOutputLocation')
                        ...
                        error('bioinfo:getblast:HTMLWriteError',...
                            'Unable to write to ''%s''. Received error: %s.', theException.message);
                    end
                end
            else
                ...
                if fid == (-1)
                    ...
                    if strcmp(format,'&FORMAT_TYPE=xml')
                        blasttext(1:regexpi(blasttext,'\?xml version=".+"?>','once')-2)=[];
                        blasttext(regexpi(blasttext,'</BlastOutput>','end')+1:end)=[];
                        blasttext=regexprep(blasttext, '(<[^>-]+)-([^>]+>)', '$1_$2');
                    end
...
if strcmp(format,'&FORMAT_TYPE=text')
