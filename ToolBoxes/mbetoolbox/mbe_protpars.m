function varargout = mbe_protpars(varargin)
%MBE_PROTPARS - M-file for mbe_protpars.fig
%      MBE_PROTPARS, by itself, creates a new MBE_PROTPARS or raises the existing
%      singleton*.
%
%      H = MBE_PROTPARS returns the handle to a new MBE_PROTPARS or the handle to
%      the existing singleton*.
%
%      MBE_PROTPARS('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in MBE_PROTPARS.M with the given input arguments.
%
%      MBE_PROTPARS('Property','Value',...) creates a new MBE_PROTPARS or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before mbe_protpars_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to mbe_protpars_OpeningFcn via varargin.
%
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @mbe_protpars_OpeningFcn, ...
                   'gui_OutputFcn',  @mbe_protpars_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before mbe_protpars is made visible.
function mbe_protpars_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to mbe_protpars (see VARARGIN)

% Choose default command line output for mbe_protpars
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes mbe_protpars wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = mbe_protpars_OutputFcn(hObject, eventdata, handles)
...
optionstr = '';
i_GeneticCode = get(handles.GeneticCode, 'Value');
optionstr = [optionstr, ' -c', num2str(i_GeneticCode)];
...
if (i_threshYES==1)
 ...
 if (exist('mbe_protpars.fig','file')==2 && exist('mbe_protpars.m','file')==2),

 dirstr=chdir2where('mbe_protpars.m');
if (ispc)
		cmd = 'mbe_protpars.exe';
...
% --- Executes during object creation, after setting all properties.
function GeneticCode_CreateFcn(hObject, eventdata, handles)
% hObject    handle to GeneticCode (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on selection change in GeneticCode.
function GeneticCode_Callback(hObject, eventdata, handles)
% hObject    handle to GeneticCode (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: contents = get(hObject,'String') returns GeneticCode contents as cell array
%        contents{get(hObject,'Value')} returns selected item from GeneticCode
