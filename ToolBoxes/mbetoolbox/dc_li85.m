function [dS,dN,dN_dS,L0,L2,L4,D0,D2,D4]=dc_li85(aln)
%DC_LIO85 - Compute syn. non-syn. substitutions rates by Li85 method
...
%nonsynonymous sites (Li, Wu and Luo 1985).
%
% Syntax: [dS,dN,dN_dS,L0,L2,L4,D0,D2,D4]=dc_li85(aln)
%
% Inputs:
...
if (hasgap(seq))
	disp ('WARNING: Gaps in sequences have been removed.x')
	seq=rmcodongaps(seq);
end
[n,m]=size(seq);
...
%	a0=1/(1-2*p0-q0); b0=1/(1-2*q0);	% c0=(a0-b0)/2; k0=(a0+b0)/2;
%	a2=1/(1-2*p2-q2); b2=1/(1-2*q2);	% c2=(a2-b2)/2; k2=(a2+b2)/2;
%	a4=1/(1-2*p4-q4); b4=1/(1-2*q4);	% c4=(a4-b4)/2; k4=(a4+b4)/2;
	...
	dS(i,j) = x2real(3*(m2*A2+m4*(A4+B4))/(m2+3*m4));
	dN(i,j) = x2real(3*(m2*B2+m0*(A0+B0))/(2*m2+3*m0));
