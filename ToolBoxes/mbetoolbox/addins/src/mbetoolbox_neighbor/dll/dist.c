    ...
    for (j = 0; j < spp; j++) {
      ...
      if (!skipit) {
        ...
        if (fscanf(infile, "%lf", &x[i][j]) != 1)  {
          mexPrintf("The infile is of the wrong type\n");
          exxit(-1);
        }
        if (replicates) {
          if (eoln(infile))
      ...
      if ((i == j) && (fabs(x[i][j]) > 0.000000001)) {
       mexPrintf("\nERROR: diagonal element of row %ld of distance matrix ", i+1);
        mexPrintf("is not zero.\n");
        mexPrintf("       Is it a distance matrix?\n\n");
        exxit(-1);        
      }
      if ((j < i) && (fabs(x[i][j]-x[j][i]) > 0.000000001)) {
        mexPrintf("ERROR: distance matrix is not symmetric:\n");
        mexPrintf("       (%ld,%ld) element and (%ld,%ld) element are unequal.\n",
               i+1, j+1, j+1, i+1);
        mexPrintf("       They are %10.6f and %10.6f, respectively.\n",
               x[i][j], x[j][i]);
        mexPrintf("       Is it a distance matrix?\n\n");
        exxit(-1);
      }
...
void treeout(node *p, long *col, double m, boolean njoin, node *start)
{
  ...
  if (p == start)
      ...
      /* The next few lines modified to make a 'real' rooted tree -- James J. Cai 6/6/2006 */  
	 if (p->back==start)
        fprintf(outtree, ":%*.5f", (int) w + 7, x*.5);
	 else
	    fprintf(outtree, ":%*.5f", (int) w + 7, x);
	*col += w + 8;
  }
