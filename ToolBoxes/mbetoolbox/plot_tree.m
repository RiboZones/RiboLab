...
% Adopted from: RE Strauss
  ...
  if (min(size(anc)) > 1)
    error('PLOT_TREE: ancestor function must be a vector.');
  end; 
  ...
  i = find(anc==0);
  if (isempty(i))
    error('PLOT_TREE: ancestor of root node must be specified as 0');
  end;
...
%    clf;
    hold on;                    % Plot tree
      ...
      if (vert & ~isempty(labels))
        disp('PLOT_TREE warning: taxon labels ignored for vertical tree');
      end;
...
%      axis('off');
  ...
  if (~isempty(brlen))          
    if (min(size(brlen)) > 1)
      error('PLOT_TREE: branch lengths must be specified as a vector.');
    end; 
