...
%             results.Original_Index=Index;
            ...
            if length(results)==1
                ...
                switch settings.SearchMethod
                    case 'StrictLinear'
                        ...
                        NewMatch_Index=num2cell([results(1).Pattern_Index{nnP(keepPf)}]);
        ...
        function Identity(results,PA,pattern,Residue_list)
            num_Res=length(Residue_list);
            residue_list=zeros(num_Res,1);
            for j=1:num_Res
                residue_list(j)=str2double(cell2mat(regexp(Residue_list{j},'\d','match')));
            end
