...
%                     '_Def_Contour.eps']);
        ...
        function ResidueList = AddMapStructureFile(map_object,file_name,MoleculeChainMap)
            ...
            numDataPoints=size(MapData.data,1);
            map_object.ItemNames=cell(numDataPoints,1);
            map_object.X=zeros(numDataPoints,1);
            map_object.Y=zeros(numDataPoints,1);
            for i=1:numDataPoints
                mol_name=regexp(MapData.textdata{i+1,1},'([^:]+):','tokens');
                chainID=MoleculeChainMap.(['mol_',mol_name{1}{1}]);
                map_object.ItemNames{i}=regexprep(MapData.textdata{i+1,1},'([^:]+):',[chainID,'_']);
                map_object.X(i)=MapData.data(i,1);
                map_object.Y(i)=MapData.data(i,2);
                map_object.Other(i,1).Letter=MapData.textdata{i+1,2};
            end
            ResidueList = MapData.textdata(2:end,1);
        end
