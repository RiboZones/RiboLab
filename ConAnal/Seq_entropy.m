function [ Mutation_Entropy, Shannon_Entropy, F, classes ] = Seq_entropy(Alignment,varargin)
...
switch lower(Method)
    case 'maxfrequency'
        Mutation_Entropy={max(F)};
        Shannon_Entropy='notcomputed';
        return
    case 'shannon'
...
if strcmp(gap_mode,'FilterGaps')
    Shannon_Entropy(1-sum(F) > gap_filter)=-1;
end
