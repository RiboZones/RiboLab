function d=OnionPlot(Pro,bins,keep,doubled)
...
if nargin <3
    keep.PDB=1:length(Pro.shell);    
    keep.Alignment=keep.PDB;
end
if nargin <4
    doubled=false;
end
...
figure();
numshells=length(unique(Pro.shell(keep.PDB)));
minShell=min(unique(Pro.shell(keep.PDB)));
G=cell(1,numshells);
H=cell(1,numshells);
...
ylim([0,1.19])
title(Pro.Name,'Interpreter','none','FontName','Myriad Pro','FontSize',20)
...
reverse=get(gca,'Children');
set(gca,'Children',reverse(end:-1:1))
legend(binName(end:-1:1),'FontName','Myriad Pro','FontSize',16,'Color',[.8,.8,.8])
